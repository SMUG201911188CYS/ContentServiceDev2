...
<body>
	...
	<div class = "input-group">
		<form action = "Seven_search.php" method = "get">
	...
	// MySQL 검색 실행 및 결과 출력
		$query = "select distinct sale_product.convenience, product.name, product.price, sale_product.event_type, sale_product.event_info, product.image, product.id, product.type FROM sale_product, product where sale_product.id = product.id AND product.name like '%".$search."%' AND sale_product.event_type LIKE '%".$plus."%' AND product.type LIKE '%".$type."%' AND sale_product.convenience = '7-ELEVEn' order by CONVERT(product.price, signed) ".$price.";";
	...
	//AND sale_product.id = product.id AND sale_product.sale_type LIKE '%".$plus."%' AND product.type LIKE '%".$type."%';";
		...
		<br><br><div class = "GS_gather">
			...
			?>
			<div class = "menu_gs">
				<small class ="seleven_pn_name"><?php echo $row["convenience"]; ?>
